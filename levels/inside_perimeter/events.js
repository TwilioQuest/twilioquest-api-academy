...
const CLEAR_STATE = {
  ...
  insideCatacombs: {
    keySpellsObtained: ['lovelace_flame', 'hopper_flame', 'turing_flame', 'neumann_flame'],
    hasCredentials: false,
    hasKey: false,
    hasPledgeScroll: false,
    tweenRunning: false
...
module.exports = async function (event, world) {
  ...
  const objectNotifications = {
    ...
    portrait_hopper: 'I heard that some sneaky students make use of Postman variables to keep track of their credentials across requests. Can you believe it?',
    portrait_lovelace: 'Every SMS must go to someone... could it really be that you\'re messaging me? It\'s been so long since anyone has sent a missive my way...',
    portrait_neumann: 'A whisper in the wind said that students who use GET requests to send messages with the Twilio Programmable SMS API aren\'t getting the results they want.',
    portrait_turing: 'Those lucky JavaScript students... Twilio gives them server-side helper libraries and browser SDKs too?',
    statue_hopper: 'In House Hopper, you will find that knowing how to weild tools is the best way to achieve success. For example, to send authorization credentials in an API request in Postman, you must use the Authorization tab. This knowledge will magnify the magic of House Hopper.',
    statue_lovelace: 'In House Lovelace, you will learn that understanding is key to expression. To enhance the power of House Lovelace, explore an API\'s documentation to find the answers you seek.',
    statue_neumann: 'In House Neumann, you will discover that efficiency and logic are at the core of your magic. Knowing that GET requests are used to retrieve information and POST requests are used to send information is necessary to maximize the efficiency of your API code.',
    statue_turing: 'In House Turing, you will find that humans are at the core of all code. Humans of House Turing specialize in creating Software Development Kits and Libraries to make working with APIs easier for students of every programming language.',
  ...
  const unlock = event => {
    if (!worldState.insideCatacombs.hasKey) { 
      world.showNotification("You need to the magic key to unlock the Scroll Room. Conquer the objectives inside these catacombs before returning."); 
      return; 
    }
  ...
  const runSpell = event => {
    if (!worldState.insidePerimeter.hasWand) { 
      world.showNotification("Go find the toolshed and see if there is an extra wand inside!"); 
      return; 
    }
  ...
  const addMagicKey = event => {
    ...
    if (!worldState.spellsEarned.includes('unlock')) worldState.spellsEarned.push('unlock');
    destroyObject('magic_key');
    world.showNotification('You\'ve obtained the magic key, go forth and claim your pledge scroll.');
  ...
  const addPledgeScroll = event => {
    ...
    world.showNotification('You\'ve obtained your pledge scroll. Head to the Academy building and present it to the headmaster.');
  ...
  const determineHouse = () => {
    ...
    const bestScore = Math.min(...worldState.houses.map(house => house.magicScore))
    const bestHouses = worldState.houses.filter(house => house.magicScore == bestScore);
  ...
  if (event.name === "mapDidLoad") {
    ...
    world.destroyEntities(({instance}) => worldState.destroyedEntities.includes(instance.group || instance.key));

    // show all previously unlocked objects
    world.showEntities(({instance}) => worldState.unlockedEntities.includes(instance.group || instance.key));
    ...
    world.forEachEntities(({instance}) => worldState.unhackableEntities.includes(instance.group || instance.key), entity => {
      entity.hackable = false;
    });
